...
train = schnetpack.data.AtomsData('dataset_10_12_train_combined.db', available_properties=['KD'],
                                  environment_provider=schnetpack.environment.TorchEnvironmentProvider(10.,
                                                                                                       torch.device(
                                                                                                           'cpu')))
if (len(train) == 0):
    PreprocessingSchnet.createDatabase(train, threshold=12, data_path='../Data/combined1618/',
                                       index_path='../../Data/INDEX_refined_data.2016.2018')

bench = schnetpack.data.AtomsData('dataset_10_12_test.db', available_properties=['KD'],
                                  environment_provider=schnetpack.environment.TorchEnvironmentProvider(10.,
                                                                                                       torch.device(
                                                                                                           'cpu')))
if (len(bench) == 0):
    PreprocessingSchnet.createDatabase(bench, data_path='../../Data/test/', threshold=12)

train, val, test = schnetpack.train_test_split(data=train, num_val=750, num_train=len(train) - 750)

print(len(train), len(bench), len(val))

# Create Dataloader for Training
train_loader = schnetpack.AtomsLoader(train, batch_size=8, shuffle=True)
val_loader = schnetpack.AtomsLoader(val, batch_size=1)
...
model = schnetpack.representation.SchNet(use_noise=False, noise_mean=0.0, noise_std=0.1,
...
d = schnetpack.atomistic.Atomwise(n_in=20, aggregation_mode='avg',
                                  n_layers=4)  # , activation=F.relu)#, output_activation=act.forward)
...
hooks = [schnetpack.train.CSVHook(log_path=Resultfolder, metrics=metrics),
         schnetpack.train.ReduceLROnPlateauHook(optimizer, patience=15, factor=0.5, min_lr=1e-5),
         schnetpack.train.EarlyStoppingHook(patience=45,
...
trainer = schnetpack.train.Trainer(model_path=Resultfolder, model=model, loss_fn=loss, train_loader=train_loader,
                                   ...
                                   keep_n_checkpoints=10, checkpoint_interval=5)
